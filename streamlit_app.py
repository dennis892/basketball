...
我將根據上述的優化建議，直接幫你修改程式碼。這段程式碼將會包含以下變動：

1.  **統一深色主題：** 強化現有的深色藍主題，並讓 `st.info` 和 `st.warning` 的顏色與整體風格更協調。
2.  **提升資訊層次：** 使用 `st.title`、`st.header` 和 `st.subheader` 讓頁面結構更清晰。
3.  **優化資訊呈現：**
      * 在「球員資訊」頁面，使用 `st.columns` 讓球員頭像與基本資料並排顯示。
      * 使用 `st.metric` 呈現關鍵數據（比賽場數、命中率等），視覺效果更醒目。
      * 將勳章統計也使用 `st.columns` 排版，讓畫面更緊湊。
4.  **改善表單設計：** 在「新增球員」表單中使用 `st.columns` 分欄排版，讓使用者填寫時更輕鬆。

請直接用這段程式碼替換你原有的 `main.py` 內容，然後執行看看效果。
